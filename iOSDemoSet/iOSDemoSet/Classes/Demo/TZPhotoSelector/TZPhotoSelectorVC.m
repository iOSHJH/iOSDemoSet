...
@interface TZPhotoSelectorVC ()<TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate>
...
@implementation TZPhotoSelectorVC
...
- (void)initPhotoSelector {
    ...
    self.allowPickingMuitlpleVideoSwitch = YES;
    self.showSelectedIndexSwitch = YES;
    if (self.margin < 1) {
        _margin = 4;
    }
    if (self.itemWH < 1) {
        if (self.itemColumnCount < 1) {
            self.itemColumnCount = 3;
        }
//        _itemWH = (self.view.tz_width - 3 * _margin - _margin) / 3 - _margin;
        _itemWH = (ScreenWidth - (self.itemColumnCount-1) * _margin - _margin) / self.itemColumnCount - _margin;
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.item == _selectedPhotos.count) {
        cell.imageView.image = [UIImage imageNamed:@"icon-tianjiazhaopian"];
...
//                self->_collectionView.contentSize = CGSizeMake(0, ((self->_selectedPhotos.count + 2) / 3 ) * (self->_margin + self->_itemWH));
                self->_collectionView.contentSize = self.view.bounds.size;
            }];
